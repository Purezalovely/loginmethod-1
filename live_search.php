...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['search'])) {
        ...
        try {
            ...
            if ($connection) {
                // SQL query with JOIN
                $query = $connection->prepare("SELECT users.user_id, users.firstname, users.lastname, users.birthday, users.sex, users.username, users.profile_picture, CONCAT(user_address.city,', ', user_address.province) AS address FROM users INNER JOIN user_address ON users.user_id = user_address.user_id WHERE users.user_name LIKE ? OR users.user_id LIKE ?");
                ...
                foreach ($users as $user) {
                    ...
                    $html .= '<td><img src="' . htmlspecialchars($user['profile_picture']) . '" alt="Profile Picture" style="width: 50px; height: 50px; border-radius: 50%;"></td>';
